...
bool OMXReader::getHints(AVStream *stream, StreamInfo *hints)
{
    if(!hints || !stream)
        return false;
...
bool OMXReader::getHints(OMXStreamType type, unsigned int index, StreamInfo &hints)
{
...
bool OMXReader::getHints(OMXStreamType type, StreamInfo &hints)
{
    bool ret = false;
...
std::string OMXReader::getStreamType(OMXStreamType type, unsigned int index)
{
    ...
    for(int i = 0; i < MAX_STREAMS; i++)
    {
        if(omxStreams[i].type == type &&  omxStreams[i].index == index)
        {
            if (omxStreams[i].hints.codec == AV_CODEC_ID_AC3) strcpy(sInfo, "AC3 ");
            else if (omxStreams[i].hints.codec == AV_CODEC_ID_DTS)
            {
#ifdef FF_PROFILE_DTS_HD_MA
            ...
            else if (omxStreams[i].hints.codec == AV_CODEC_ID_MP2) strcpy(sInfo, "MP2 ");
            else strcpy(sInfo, "");
